...
- **Local files** → write one `tickets.md` under `{agentRoot}/work/<feature-slug>/`, all tickets in dependency order (blockers first), each with its "Blocked by" listing the titles it depends on. `{agentRoot}` comes from the `## Agent skills` block in `AGENTS.md`/`CLAUDE.md`. Use the file template below.
